...
export function EditHearingPage() {
  ...
  const fetchHearing = async () => {
    try {
      const response = await fetch(`http://localhost:8000/api/hearings/${id}/`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('access_token')}`,
          'Content-Type': 'application/json'
        }
      ...
      toast.error("Failed to fetch hearing");
      navigate('/hearings');
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiService.updateHearing(Number(id), formData);
      toast.success("Hearing updated successfully");
      navigate('/hearings', { replace: true });
